...
The model permission assumption is not the blocker. The current browser runtime still fails the required ten-bot/state bridge: Tomb’s JSPI build is stable for unmodded FreeDM, but aborts when a `-file` bridge PK3 is mounted, and its launched multiplayer session reports one available player slot. Therefore this repository does not add the public `/doom/` route or homepage card yet. If the GZDoom runtime is repaired, GPLv3 source/distribution obligations for the port must be preserved.
